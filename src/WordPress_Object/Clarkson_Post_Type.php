...
class Clarkson_Post_Type {
	...
	/**
	 * @deprecated Use $this->get_post_type() instead.
	 *
	 * @var \WP_Post_Type
	 */
	...
	public function __get( string $name ) {
		if ( property_exists( $this->get_post_type(), $name ) ) {
			return $this->get_post_type()->$name;
		}
	...
	public function get_posts( $args ) {
		$args['post_type']     = $this->get_post_type()->name;
	...
	public function __isset( string $name ): bool {
		return property_exists( $this->get_post_type(), $name );
	}
	...
	public function get_title(): string {
		$title = $this->get_post_type()->labels->singular_name;
		return $title;
	}
	...
	public function get_archive_title(): string {
		$title = apply_filters( 'post_type_archive_title', $this->get_post_type()->labels->name, $this->get_post_type()->name );
	...
	public function get_archive_permalink(): ?string {
		$link = get_post_type_archive_link( $this->get_post_type()->name );
		if ( $link ) {
			return $link;
		}
		return null;
	...
	public function get_archive_feed_permalink( string $feed = '' ): ?string {
		$link = get_post_type_archive_feed_link( $this->get_post_type()->name, $feed );
		if ( $link ) {
			return $link;
		}
		return null;
